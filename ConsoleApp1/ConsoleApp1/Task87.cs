class Task87
{
    public void Do()
    {
        ...
        do
        {
            Console.WriteLine("User 1.Determine the range of numbers to play with");
            Console.WriteLine("Enter the MIN number");
            var userMinNumber = Console.ReadLine();
            isNumber2 = int.TryParse(userMinNumber, out MIN_NUMBER);
            if (!isNumber2)
            {
        ...
        do
        {
            ...
            var userMaxNumber = Console.ReadLine();
            isNumber3 = int.TryParse(userMaxNumber, out MAX_NUMBER);
            if (!isNumber3)
            {
                Console.WriteLine("It's not a number");
            } 
        ...
        do
        {
            ...
            Console.WriteLine($"User 2. Enter your guess. Attemmpt [{attempt} / {MAX_ATTEMPT}]. The range of numbers is from [{MIN_NUMBER}] to [{MAX_NUMBER}]");
            var guessText = Console.ReadLine();
            guess = int.Parse(guessText);

            if (guess < userMagicNumber && guess < MIN_NUMBER)
            {
                attempt--;
            }
            else if (guess < userMagicNumber && guess > MIN_NUMBER)
            {
